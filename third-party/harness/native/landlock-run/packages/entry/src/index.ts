 ...
 * npm package `@cimdev/harness/node-addon-landlock-run-<platform>-<arch>` (npm's
...
export function launcherPath(
  ...
  const platformPackage = `@cimdev/harness/node-addon-landlock-run-${process.platform}-${process.arch}`
  try {
