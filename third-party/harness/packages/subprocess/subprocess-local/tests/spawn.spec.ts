...
import type { SubprocessHandle, SubprocessOutputReader } from '@cimdev/harness/dsh-subprocess'
import { MAX_TIMER_DELAY_MS } from '@cimdev/harness/dsh-timeout'
...
describe('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@cimdev/harness/cordis')
    const { default: LocalSubprocessRuntime } = await import('@cimdev/harness/dsh-subprocess-local')
