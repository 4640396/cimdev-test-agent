...
const platformPackageName = `@cimdev/harness/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @cimdev/harness peer (koffi arrives
  // from the registry).
  ...
  // rescoped into @cimdev/harness, so the consumer installs this repository's
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@cimdev/harness/node-addon-landlock-run'
